...
func TestNewHybridServer(t *testing.T) {
	type args struct {
		httpPort    int
		grpcPort    int
		apiKey      string
	...
	tests := []struct {
	...
	}{
		{
			...
			want: HybridServer{httpPort: 1234, grpcPort: 4321, apiKey: "1234", db: NewMockDB()},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := NewHybridServer(tt.args.httpPort, tt.args.grpcPort, tt.args.apiKey, tt.args.persistence)
			if (err != nil) != tt.wantErr {
				t.Errorf("NewHybridServer() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewHybridServer() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestHybridServer_auth(t *testing.T) {
	type args struct {
		ctx     context.Context
		req     interface{}
	...
	tests := []struct {
		...
		{
			name: "1",
			s:    &HybridServer{apiKey: "1234", db: NewMockDB()},
			args: args{
		...
		{
			name: "2",
			s:    &HybridServer{apiKey: "1111", db: NewMockDB()},
			args: args{
				ctx:     metadata.NewIncomingContext(context.Background(), metadata.MD{"x-api-key": []string{"1234"}}),
				req:     "request2",
				handler: func(ctx context.Context, req interface{}) (interface{}, error) { return nil, nil },
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("HybridServer.auth() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("HybridServer.auth() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestHybridServer_tracer(t *testing.T) {
	type args struct {
		ctx     context.Context
		req     interface{}
		info    *grpc.UnaryServerInfo
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("HybridServer.tracer(): %v", err)
			}
			if !match {
				t.Errorf("HybridServer.tracer() = %v, want %v", got, tt.wantRegex)
			}
		})
	}
}
