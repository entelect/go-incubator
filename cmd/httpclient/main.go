...
func main() {
	...
	httpClient, err := http.NewHttpClient(cfg.Address, cfg.HttpPort, cfg.ApiKey)
	if err != nil {
	...
	for {
		...
		case "Add a recipe":
			...
			fmt.Println()
			err = httpClient.AddRecipe(newRecipe)
			if err != nil {
		...
		case "Get a recipe":
			...
			recipe, err := httpClient.GetRecipe(name)
			if err != nil {
		...
		case "Search by ingredients":
			...
			recipes, err := httpClient.SearchByIngredients(ingredients)
			if err != nil {
